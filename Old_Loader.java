...
public class Old_Loader {
	...
	public Old_Loader(String user, String password,  
					...
					String selectedMS, String csvSeparatorString, int timeout) {
		//this.hmc = hmc;
		this.user = user;
	...
	public static void main(String[] args) {
		...
		Old_Loader l = new Old_Loader(result[USER], result[PASSWORD], 
								...
								result[SELECTED_MS], result[CSV_SEPARATOR],timeout);
		...
		*/
		...
		String dirName = result[DIR] + File.separatorChar + 
							result[HMC] + "_" +
							l.getHMCdate() + 
							html;
		...
		l.createExcel(	result[DIR] + File.separatorChar +
						result[HMC] + "_" +
						l.getHMCdate() + 
						excel, dirName, csvName);
	...
	private void collect_data(String hmc, String baseDir) {
		...
		try {
			...
			fw.write("time="+cal.get(Calendar.HOUR)+":"+cal.get(Calendar.MINUTE)+":"+cal.get(Calendar.SECOND)+",");
			fw.write("HMCdate="+hmcDate.get(Calendar.YEAR)+"-"+(hmcDate.get(Calendar.MONTH)+1)+"-"+hmcDate.get(Calendar.DAY_OF_MONTH)+",");
			fw.write("HMCtime="+hmcDate.get(Calendar.HOUR)+":"+hmcDate.get(Calendar.MINUTE)+":"+hmcDate.get(Calendar.SECOND));
			*/
		...
		switch (managerType) {
			case M_HMC:		System.out.println("HMC");
							...
							//sshm.sendCommand("lshmc -r", baseDir + lshmcr);
							sshm.sendCommand("lshmc -v", baseDir + lshmcv);
							break;
		...
		try {
			...
			while ( (line=br.readLine())!= null ) {
				...
				if (managerType == M_IVM) {
					...
					//sshm.sendCommand(prefix + "viosvrcmd -m \'"+name+"\' -p \'"+vios+"\' -c \"lspv | grep -v '^NAME' | while read i j ; do lsdev -dev $i -attr unique_id | awk -v h=$i '$1!=\"value\" && !/^$/ {printf(\"%s:%s\\n\",h,$1)}'; done", baseDir + name + "_" + vios + "_" + diskuuid);
	...
	private void parseData(String hmc, String baseDir) {
		...
		if (produceStatistics) {
			...
			msCoreConfig = new Old_DataManager[managedSystem.length];
			msCoreAvail = new Old_DataManager[managedSystem.length];
			msCoreUsed = new Old_DataManager[managedSystem.length];
			msMemConfig = new Old_DataManager[managedSystem.length];
			msMemAvail = new Old_DataManager[managedSystem.length];
			lparEnt = new Old_DataManager[lparNames.length];
			lparVP = new Old_DataManager[lparNames.length];
			lparPC = new Old_DataManager[lparNames.length];
			lparStatus = new LparStatus[lparNames.length];
			...
			for (i=0; i<managedSystem.length; i++) {
				msCoreConfig[i] = new Old_DataManager(scannerDate);
				msCoreAvail[i] = new Old_DataManager(scannerDate);
				msCoreUsed[i] = new Old_DataManager(scannerDate);
				msMemConfig[i] = new Old_DataManager(scannerDate);
				msMemAvail[i] = new Old_DataManager(scannerDate);				
			}
			for (i=0; i<lparNames.length; i++) {
				lparEnt[i] = new Old_DataManager(scannerDate);
				lparVP[i] = new Old_DataManager(scannerDate);
				lparPC[i] = new Old_DataManager(scannerDate);	
				lparStatus[i] = new LparStatus(scannerDate);
			}
	...
	private void load_all_stats(String baseDir) {
		...
		for (i=0; i<managedSystem.length; i++) {
			...
			load_single_stats(i, baseDir + sysName + "_" + lslparutilHourlyStats);
			msCoreConfig[i].endOfHourlyData();
			msCoreAvail[i].endOfHourlyData();
			msCoreUsed[i].endOfHourlyData();
			msMemConfig[i].endOfHourlyData();
			msMemAvail[i].endOfHourlyData();		
		}
		
		for (i=0; i<lparNames.length; i++) {
			lparEnt[i].endOfHourlyData();
			lparVP[i].endOfHourlyData();
			lparPC[i].endOfHourlyData();
			lparStatus[i].endOfHourlyData();
		}
		
		for (i=0; procPoolName!=null && i<procPoolName.length; i++) {
			procPoolConfig[i].endOfHourlyData();
			procPoolUsed[i].endOfHourlyData();
		}
		
		for (i=0; i<managedSystem.length; i++) {
			sysName = managedSystem[i].getVarValues("name")[0];
			load_single_stats(i, baseDir + sysName + "_" + lslparutilDailyStats);
			msCoreConfig[i].endOfDailyData();
			msCoreAvail[i].endOfDailyData();
			msCoreUsed[i].endOfDailyData();
			msMemConfig[i].endOfDailyData();
			msMemAvail[i].endOfDailyData();		
		}
		
		for (i=0; i<lparNames.length; i++) {
			lparEnt[i].endOfDailyData();
			lparVP[i].endOfDailyData();
			lparPC[i].endOfDailyData();
			lparStatus[i].endOfDailyData();
		}
		
		for (i=0; procPoolName!=null && i<procPoolName.length; i++) {
			procPoolConfig[i].endOfDailyData();
			procPoolUsed[i].endOfDailyData();
		}
	...
	private int getProcPoolId(String ms, String poolName) {
		...
		if (procPoolName == null) {
			...
			procPoolName[0]=s;
			procPoolConfig = new Old_DataManager[1];
			procPoolConfig[0] = new Old_DataManager(scannerDate);
			procPoolUsed = new Old_DataManager[1];
			procPoolUsed[0] = new Old_DataManager(scannerDate);
			return 0;
		}
		...
		if (n==procPoolName.length) {
			...
			Old_DataManager oldPoolConfig[] = procPoolConfig;
			Old_DataManager oldPoolUsed[] = procPoolUsed;
			
			procPoolName = new String[procPoolName.length+1];
			procPoolConfig = new Old_DataManager[procPoolConfig.length+1];
			procPoolUsed = new Old_DataManager[procPoolUsed.length+1];
			...
			procPoolName[n] = s;
			procPoolConfig[n] = new Old_DataManager(scannerDate);
			procPoolUsed[n] = new Old_DataManager(scannerDate);
	 ...
	 */
	private void load_single_stats(int num, String fileName) {
			...
			while ( (line=br.readLine())!= null ) {
				...
				if (resource_type.equals("pool")) {
					...
					if ( prevPoolSample != null &&
						...
						msCoreUsed[num].add(prevPoolSample, bi3.floatValue());
					}
				...
				if (resource_type.equals("procpool")) {
					...
					if (	prevProcPoolData[poolID][UTILIZED_POOL_CYCLES]!=null &&
						...
						procPoolConfig[id].add(prevProcPoolSample[poolID], bi2.floatValue()); // Add pool Size
				...
				if (resource_type.equals("lpar")) {
					...
					lparStatus[id].add(sample, managedSystem[num].getVarValues("name")[0], poolName, cap);
					...
					if ( prevLparSample[id] != null  &&
						...
						if (bi2.compareTo(zero)!=0) {
							bi3=bi1.divide(bi2,new MathContext(5));
							lparPC[id].add(prevLparSample[id], bi3.floatValue());
						}		
					}
	...
	private DataSheet createCPUSheetRowBased() {
		...
		n = ds.addLabel(col,row,"E:VP %",BOLD|CENTRE|VCENTRE|B_ALL_MED|GREEN);		 if (n>size[col]) size[col]=n; col++;
	...
	private DataSheet createSysRAMUsageSheet() {
		...
		GregorianCalendar[] gcNames=null;
		
		for (i=0; i<managedSystem.length; i++)
			if ( (gcNames=msMemConfig[i].getMonthNames()) !=null)
				break;
		...
		for (i=0; i<gcNames.length; i++) {
			s = gcNames[i].get(Calendar.YEAR)+"-"+(gcNames[i].get(Calendar.MONTH)+1)+"-"+gcNames[i].get(Calendar.DAY_OF_MONTH);
		...
		for (i=0; i<managedSystem.length; i++) {
			...
			for (j=0; j<gcNames.length; j++) {
				f = msMemAvail[i].getMonthAvg(j);
				f2 = msMemConfig [i].getMonthAvg(j);
				if (f>=0)
	...
	private DataSheet createLparDailyUsageSheet(int from, int to) {
		...
		// Check that at least one systems has data
		for (i=from; i<=to; i++)
			if ( (gc=lparPC[i].getDayDate(0)) !=null)
				break;
		...
		for (i=0; i<365; i++) {
			gc = lparPC[validLPAR].getDayDate(i);
			s = gc.get(Calendar.YEAR)+"-"+(gc.get(Calendar.MONTH)+1)+"-"+gc.get(Calendar.DAY_OF_MONTH);
		...
		for (i=from; i<=to; i++) {			
			for (j=0; j<365; j++) {
				f = lparPC[i].getDayAvg(j);
				
				ent = lparEnt[i].getDayAvg(j);
				cap = lparStatus[i].getDayCap(j);
				ms = lparStatus[i].getDayMS(j);
				pool = lparStatus[i].getDayPool(j);
				vp = lparVP[i].getDayAvg(j);
				
				if (f<0 || ent<=0)
					continue;
				
				if (pool.equals("DefaultPool")) {
					free = msCoreConfig[getMSid(ms)].getDayAvg(j) - msCoreUsed[getMSid(ms)].getDayAvg(j);
				} else {
					poolID = getProcPoolId(ms,pool);
					free = procPoolConfig[poolID].getDayAvg(j)-procPoolUsed[poolID].getDayAvg(j);
				}
		...
		for (i=from; i<=to; i++) {
			...
			if (num[i-from]==0) {
				...
				for (j=364; j>=0; j--)
					if ( lparEnt[i].getDayAvg(j)>0) break;
				ds.addFloat(1+i-from,row-3, lparEnt[i].getDayAvg(j), B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW);		
			}
		}
	...
	private void createLparDailyUsageSheet(WritableSheet sheet, int from, int to) throws RowsExceededException, WriteException {
		int i,j;
		String s;
		float f;
		int		size=0;
		...
		addLabel(sheet,0,row,"Red",formatLabel(BOLD|CENTRE|VCENTRE|RED));
		addLabel(sheet,1,row,"Usage >= "+R_LEVEL+"% of max allowed size",formatLabel(BOLD));
		row++;
		
		addLabel(sheet,0,row,"Yellow",formatLabel(BOLD|CENTRE|VCENTRE|YELLOW));
		addLabel(sheet,1,row,"Usage >= "+Y_LEVEL+"% of max allowed size",formatLabel(BOLD));
		row++;
		
		addLabel(sheet,0,row,"Bold",formatLabel(BOLD|CENTRE|VCENTRE));
		addLabel(sheet,1,row,"Usage > entitled capacity",formatLabel(BOLD));
		row++;
		
		addLabel(sheet,0,row,"LPAR color",formatLabel(BOLD|CENTRE|VCENTRE));
		addLabel(sheet,1,row,"Colored cells >= "+COLOR_LEVEL*100+"% of total (yellows include reds)",formatLabel(BOLD));
		row++;
		row++;
		
		sheet.mergeCells(0, row, to-from+2, row);
		addLabel(sheet,0,row,"Average LPAR Usage",formatLabel(BOLD|LEFT|VCENTRE|GREEN));
		row++;
		row++;
		
		addLabel(sheet,0,row,"Days",formatLabel(BOLD|VCENTRE|CENTRE|B_ALL_MED));
		row++;
		addLabel(sheet,0,row,"Reds %",formatLabel(BOLD|VCENTRE|CENTRE|B_ALL_MED));
		row++;
		addLabel(sheet,0,row,"Yellows %",formatLabel(BOLD|VCENTRE|CENTRE|B_ALL_MED));
		row++;
		addLabel(sheet,0,row,"Over Cap %",formatLabel(BOLD|VCENTRE|CENTRE|B_ALL_MED));
		row++;
		addLabel(sheet,0,row,"Max",formatLabel(BOLD|VCENTRE|CENTRE|B_ALL_MED));
		row++;
		addLabel(sheet,0,row,"Avg",formatLabel(BOLD|VCENTRE|CENTRE|B_ALL_MED));
		row++;
		addLabel(sheet,0,row,"StdDev",formatLabel(BOLD|VCENTRE|CENTRE|B_ALL_MED));
		row++;
		addLabel(sheet,0,row,"90 Perc",formatLabel(BOLD|VCENTRE|CENTRE|B_ALL_MED));
		row++;
		addLabel(sheet,0,row,"Last Ent",formatLabel(BOLD|VCENTRE|CENTRE|B_ALL_MED));
		row++;
		row++;
		...
		for (i=from; i<=to; i++) {
			...
			addLabel(sheet,1+i-from,row,s,formatLabel(BOLD|DIAG45));
		}
		*/
		...
		for (i=0; i<365; i++) {
			gc = lparPC[validLPAR].getDayDate(i);
			s = gc.get(Calendar.YEAR)+"-"+(gc.get(Calendar.MONTH)+1)+"-"+gc.get(Calendar.DAY_OF_MONTH);
			addLabel(sheet,0,row+i,s,formatLabel(BOLD|B_ALL_MED));
		}	
		...
		for (i=from; i<=to; i++) {			
			for (j=0; j<365; j++) {
				...
				wcf = formatFloat(map);	
				addNumber(sheet,1+i-from,row+j, f, wcf);
		...
		for (i=from; i<=to; i++) {
			...
			addLabel(sheet,1+i-from,row-12,s,formatLabel(map));
			
			map = B_RIGHT_LOW|B_LEFT_LOW|B_BOTTOM_LOW;
			addNumber(sheet,1+i-from,row-11, num[i-from], formatInt(map));
			
			if (num[i-from]==0) {
				map = B_RIGHT_LOW|B_LEFT_LOW|B_BOTTOM_LOW;
				addNumber(sheet,1+i-from,row-10, 0, formatFloat(map));
				addNumber(sheet,1+i-from,row-9, 0, formatFloat(map));
				addNumber(sheet,1+i-from,row-8, 0, formatFloat(map));
				addNumber(sheet,1+i-from,row-7, 0, formatFloat(map));
				addNumber(sheet,1+i-from,row-6, 0, formatFloat(map));
				addNumber(sheet,1+i-from,row-5, 0, formatFloat(map));
				addNumber(sheet,1+i-from,row-4, 0, formatFloat(map));
				addNumber(sheet,1+i-from,row-3, 0, formatFloat(map));
			} else {		
				if (1d*red[i-from]/num[i-from]>COLOR_LEVEL)
					map = map | RED;
				addNumber(sheet,1+i-from,row-10, 100d*red[i-from]/num[i-from], formatFloat(map));
				
				map = B_RIGHT_LOW|B_LEFT_LOW|B_BOTTOM_LOW;
				if (1d*yellow[i-from]/num[i-from]>COLOR_LEVEL)
					map = map | YELLOW;
				addNumber(sheet,1+i-from,row-9, 100d*yellow[i-from]/num[i-from], formatFloat(map));
				
				map = B_RIGHT_LOW|B_LEFT_LOW|B_BOTTOM_LOW;
				addNumber(sheet,1+i-from,row-8, 100d*uncap[i-from]/num[i-from], formatFloat(map));	
				
				col = jxl.CellReferenceHelper.getColumnReference(1+i-from);
				formula = new Formula(1+i-from, row-7, "MAX("+col+(row+1)+":"+col+(row+365)+")",formatFloat(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));
				sheet.addCell(formula); 
				formula = new Formula(1+i-from, row-6, "AVERAGE("+col+(row+1)+":"+col+(row+365)+")",formatFloat(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));
				sheet.addCell(formula); 
				formula = new Formula(1+i-from, row-5, "STDEVP("+col+(row+1)+":"+col+(row+365)+")",formatFloat(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));
				sheet.addCell(formula); 
				formula = new Formula(1+i-from, row-4, "1282/1000*"+col+(row-5+1)+"+"+col+(row-6+1)+")",formatFloat(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));
				sheet.addCell(formula);
				
				for (j=364; j>=0; j--)
					if ( lparEnt[i].getDayAvg(j)>0) break;
				addNumber(sheet,1+i-from,row-3, lparEnt[i].getDayAvg(j), formatFloat(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));		
			}
		}
	...
	private DataSheet createLparHourlyUsageSheet(int from, int to) {
		...
		for (i=from; i<=to; i++) {
			...
			ds.addLabel(1+i-from,row,s,BOLD|DIAG45);
		}
		*/
		
		
		for (i=0; i<60*24; i++) {
			gc = lparPC[validLPAR].getHourDate(i);
			s = gc.get(Calendar.YEAR)+"-"+(gc.get(Calendar.MONTH)+1)+"-"+gc.get(Calendar.DAY_OF_MONTH) +
		...
		for (i=from; i<=to; i++) {			
			for (j=0; j<60*24; j++) {
				f = lparPC[i].getHourAvg(j);
				
				ent = lparEnt[i].getHourAvg(j);
				cap = lparStatus[i].getHourCap(j);
				ms = lparStatus[i].getHourMS(j);
				pool = lparStatus[i].getHourPool(j);
				vp = lparVP[i].getHourAvg(j);
				
				if (f<0 || ent<=0 || ms==null)
					continue;
				
				if (pool.equals("DefaultPool")) {
					free = msCoreConfig[getMSid(ms)].getHourAvg(j) - msCoreUsed[getMSid(ms)].getHourAvg(j);
				} else {
					poolID = getProcPoolId(ms,pool);
					free = procPoolConfig[poolID].getHourAvg(j)-procPoolUsed[poolID].getHourAvg(j);
				}
		...
		for (i=from; i<=to; i++) {
			...
			if (num[i-from]==0) {
				...
				for (j=24*60-1; j>=0; j--)
					if ( lparEnt[i].getHourAvg(j)>0) break;
				ds.addFloat(1+i-from,row-3, lparEnt[i].getHourAvg(j), B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW);		
			}
		}
	...
	private void createLparHourlyUsageSheet(WritableSheet sheet, int from, int to) throws RowsExceededException, WriteException {
		int i,j;
		String s;
		float f;
		int		size=0;
		...
		addLabel(sheet,0,row,"Hours",formatLabel(BOLD|VCENTRE|CENTRE|B_ALL_MED));
		row++;
		addLabel(sheet,0,row,"Reds %",formatLabel(BOLD|VCENTRE|CENTRE|B_ALL_MED));
		row++;
		addLabel(sheet,0,row,"Yellows %",formatLabel(BOLD|VCENTRE|CENTRE|B_ALL_MED));
		...
		*/
		...
		for (i=from; i<=to; i++) {
			...
			if (num[i-from]==0) {
				...
				col = jxl.CellReferenceHelper.getColumnReference(1+i-from);
				formula = new Formula(1+i-from, row-7, "MAX("+col+(row+1)+":"+col+(row+24*60)+")",formatFloat(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));
				sheet.addCell(formula); 
				formula = new Formula(1+i-from, row-6, "AVERAGE("+col+(row+1)+":"+col+(row+24*60)+")",formatFloat(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));
				sheet.addCell(formula); 
				formula = new Formula(1+i-from, row-5, "STDEVP("+col+(row+1)+":"+col+(row+24*60)+")",formatFloat(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));
				sheet.addCell(formula); 
				formula = new Formula(1+i-from, row-4, "1282/1000*"+col+(row-5+1)+"+"+col+(row-6+1)+")",formatFloat(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));
				sheet.addCell(formula);
				...
				for (j=24*60-1; j>=0; j--)
					if ( lparEnt[i].getHourAvg(j)>0) break;
				addNumber(sheet,1+i-from,row-3, lparEnt[i].getHourAvg(j), formatFloat(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));		
			}
		}
	...
	private DataSheet createSysPoolHourlyUsageSheet() {
		...
		// Check that at least one systems has data
		for (i=0; i<managedSystem.length; i++)
			if ( (gc=msCoreUsed[i].getDayDate(0)) !=null)
				break;
		...
		for (i=0; i<24*60; i++) {
			gc = msCoreUsed[validMS].getHourDate(i);
			s = gc.get(Calendar.YEAR)+"-"+(gc.get(Calendar.MONTH)+1)+"-"+gc.get(Calendar.DAY_OF_MONTH) +
		...
		for (i=0; i<managedSystem.length; i++) {
			...
			for (j=0; j<24*60; j++) {
				f = msCoreUsed[i].getHourAvg(j);
				f2= msCoreConfig[i].getHourAvg(j);
		...
		for (i=0; procPoolName!=null && i<procPoolName.length; i++) {	
			...
			for (j=0; j<24*60; j++) {
				f = procPoolUsed[i].getHourAvg(j);
				f2= procPoolConfig[i].getHourAvg(j);
		...
		for (i=0; i<managedSystem.length; i++) {
			...
			if (num[i]==0){
				...
				for (j=24*60-1; j>=0; j--)
					if ( msCoreConfig[i].getHourAvg(j)>0) break;
				ds.addInteger(1+i,row-3, msCoreConfig[i].getHourAvg(j), B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW);
			}
		}
		...
		for (i=0; procPoolName!=null && i<procPoolName.length; i++) {
			...
			if (num[managedSystem.length+i]==0) {
				...
				for (j=24*60-1; j>=0; j--)
					if ( procPoolConfig[i].getHourAvg(j)>0) break;
				ds.addInteger(managedSystem.length+2+i,row-3, procPoolConfig[i].getHourAvg(j), B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW);
			}
		}
		row++;
	...
	private void createSysPoolHourlyUsageSheet(WritableSheet sheet) throws RowsExceededException, WriteException {
		int i,j;
		String s;
		float f,f2;
		int		row;
		...
		addLabel(sheet,0,row,"Pool color",formatLabel(BOLD|CENTRE|VCENTRE));
		addLabel(sheet,1,row,"Colored cells >= "+COLOR_LEVEL*100+"% of total (yellows include reds)",formatLabel(BOLD));
		row++;
		row++;
		...
		sheet.mergeCells(0, row, cells, row);
		addLabel(sheet,0,row,"Average Pool Usage",formatLabel(BOLD|CENTRE|VCENTRE|GREEN));
		row++;
		...
		*/
		...
		addLabel(sheet,0,row,"90 Perc",formatLabel(BOLD|VCENTRE|CENTRE|B_ALL_MED));
		row++;
		addLabel(sheet,0,row,"Last size",formatLabel(BOLD|VCENTRE|CENTRE|B_ALL_MED));
		row++;
		row++;
		addLabel(sheet,0,row,"Date",formatLabel(BOLD|VCENTRE|CENTRE|B_ALL_MED));
		row++;
		...
		for (i=0; i<managedSystem.length; i++) {
			...
			for (j=0; j<24*60; j++) {
				...
				wcf = formatFloat(map);	
				addNumber(sheet,1+i,row+j, f, wcf);
		...
		for (i=0; procPoolName!=null && i<procPoolName.length; i++) {	
			...
			for (j=0; j<24*60; j++) {
				...
				wcf = formatFloat(map);	
				addNumber(sheet,managedSystem.length+2+i,row+j, f, wcf);
		...
		for (i=0; i<managedSystem.length; i++) {
			...
			addLabel(sheet,1+i,row-11,s,formatLabel(map));
		}
		...
		for (i=0; procPoolName!=null && i<procPoolName.length; i++) {
			...
			addLabel(sheet,managedSystem.length+2+i,row-11,s,formatLabel(map));
		}
		
		// Show days, red and yellow counters
		for (i=0; i<managedSystem.length; i++) {
			map = B_RIGHT_LOW|B_LEFT_LOW|B_BOTTOM_LOW;
			addNumber(sheet,1+i,row-10, num[i], formatInt(map));
			
			if (num[i]==0){
				addNumber(sheet,1+i,row-9, 0, formatInt(map));
				addNumber(sheet,1+i,row-8, 0, formatInt(map));
				addNumber(sheet,1+i,row-7, 0, formatInt(map));
				addNumber(sheet,1+i,row-6, 0, formatInt(map));
				addNumber(sheet,1+i,row-5, 0, formatInt(map));
				addNumber(sheet,1+i,row-4, 0, formatInt(map));
				addNumber(sheet,1+i,row-3, 0, formatInt(map));
			} else {
				if (1d*red[i]/num[i]>COLOR_LEVEL)
					map = map | RED;
				addNumber(sheet,1+i,row-9, 100d*red[i]/num[i], formatFloat(map));
				
				map = B_RIGHT_LOW|B_LEFT_LOW|B_BOTTOM_LOW;
				if (1d*yellow[i]/num[i]>COLOR_LEVEL)
					map = map | YELLOW;
				addNumber(sheet,1+i,row-8, 100d*yellow[i]/num[i], formatFloat(map));
				
				col = jxl.CellReferenceHelper.getColumnReference(1+i);
				formula = new Formula(1+i, row-7, "MAX("+col+(row+1)+":"+col+(row+24*60)+")",formatFloat(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));
				sheet.addCell(formula); 
				formula = new Formula(1+i, row-6, "AVERAGE("+col+(row+1)+":"+col+(row+24*60)+")",formatFloat(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));
				sheet.addCell(formula); 
				formula = new Formula(1+i, row-5, "STDEVP("+col+(row+1)+":"+col+(row+24*60)+")",formatFloat(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));
				sheet.addCell(formula); 
				formula = new Formula(1+i, row-4, "1282/1000*"+col+(row-5+1)+"+"+col+(row-6+1)+")",formatFloat(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));
				sheet.addCell(formula);
				

				for (j=24*60-1; j>=0; j--)
					if ( msCoreConfig[i].getHourAvg(j)>0) break;
				addNumber(sheet,1+i,row-3, msCoreConfig[i].getHourAvg(j), formatInt(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));
			}
		}
		
		for (i=0; procPoolName!=null && i<procPoolName.length; i++) {
			map = B_RIGHT_LOW|B_LEFT_LOW|B_BOTTOM_LOW;
			addNumber(sheet,managedSystem.length+2+i,row-10, num[managedSystem.length+i], formatInt(map));
			
			if (num[managedSystem.length+i]==0) {
				addNumber(sheet,managedSystem.length+2+i,row-9, 0, formatInt(map));
				addNumber(sheet,managedSystem.length+2+i,row-8, 0, formatInt(map));
				addNumber(sheet,managedSystem.length+2+i,row-7, 0, formatInt(map));
				addNumber(sheet,managedSystem.length+2+i,row-6, 0, formatInt(map));
				addNumber(sheet,managedSystem.length+2+i,row-5, 0, formatInt(map));
				addNumber(sheet,managedSystem.length+2+i,row-4, 0, formatInt(map));
				addNumber(sheet,managedSystem.length+2+i,row-3, 0, formatInt(map));
			} else {
				if (1d*red[managedSystem.length+i]/num[managedSystem.length+i]>COLOR_LEVEL)
					map = map | RED;
				addNumber(sheet,managedSystem.length+2+i,row-9, 100d*red[managedSystem.length+i]/num[managedSystem.length+i], formatFloat(map));
				
				map = B_RIGHT_LOW|B_LEFT_LOW|B_BOTTOM_LOW;
				if (1d*yellow[managedSystem.length+i]/num[managedSystem.length+i]>COLOR_LEVEL)
					map = map | YELLOW;
				addNumber(sheet,managedSystem.length+2+i,row-8, 100d*yellow[managedSystem.length+i]/num[managedSystem.length+i], formatFloat(map));
				
				col = jxl.CellReferenceHelper.getColumnReference(managedSystem.length+2+i);
				formula = new Formula(managedSystem.length+2+i, row-7, "MAX("+col+(row+1)+":"+col+(row+24*60)+")",formatFloat(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));
				sheet.addCell(formula); 
				formula = new Formula(managedSystem.length+2+i, row-6, "AVERAGE("+col+(row+1)+":"+col+(row+24*60)+")",formatFloat(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));
				sheet.addCell(formula); 
				formula = new Formula(managedSystem.length+2+i, row-5, "STDEVP("+col+(row+1)+":"+col+(row+24*60)+")",formatFloat(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));
				sheet.addCell(formula); 
				formula = new Formula(managedSystem.length+2+i, row-4, "1282/1000*"+col+(row-5+1)+"+"+col+(row-6+1)+")",formatFloat(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));
				sheet.addCell(formula);
				
				for (j=24*60-1; j>=0; j--)
					if ( procPoolConfig[i].getHourAvg(j)>0) break;
				addNumber(sheet,managedSystem.length+2+i,row-3, procPoolConfig[i].getHourAvg(j), formatInt(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));
			}
		}
		row++;
	...
	private DataSheet createSysPoolDailyUsageSheet() {
		...
		*/
		...
		for (i=0; i<365; i++) {
			gc = msCoreUsed[validMS].getDayDate(i);
			s = gc.get(Calendar.YEAR)+"-"+(gc.get(Calendar.MONTH)+1)+"-"+gc.get(Calendar.DAY_OF_MONTH);
			ds.addLabel(0,row+i,s,BOLD|B_ALL_MED);
		}		
		...
		for (i=0; i<managedSystem.length; i++) {	
			...
			for (j=0; j<365; j++) {
				f = msCoreUsed[i].getDayAvg(j);
				f2 = msCoreConfig[i].getDayAvg(j);
		...
		for (i=0; procPoolName!=null && i<procPoolName.length; i++) {	
			...
			for (j=0; j<365; j++) {
				f = procPoolUsed[i].getDayAvg(j);
				f2= procPoolConfig[i].getDayAvg(j);
		...
		for (i=0; i<managedSystem.length; i++) {
			...
			if (num[i]==0) {
				...
				for (j=364; j>=0; j--)
					if ( msCoreConfig[i].getDayAvg(j)>0) break;
				ds.addInteger(1+i,row-3, msCoreConfig[i].getDayAvg(j), B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW);
			}
		...
		for (i=0; procPoolName!=null && i<procPoolName.length; i++) {
			...
			if (num[managedSystem.length+i]==0) {
				...
				for (j=364; j>=0; j--)
					if ( procPoolConfig[i].getDayAvg(j)>0) break;
				ds.addInteger(managedSystem.length+2+i,row-3, procPoolConfig[i].getDayAvg(j), B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW);
			}
		}
	...
	private void createSysPoolDailyUsageSheet(WritableSheet sheet) throws RowsExceededException, WriteException {
		int i,j;
		String s;
		float f,f2;
		int		row;
		...
		*/
		...
		for (i=0; i<managedSystem.length; i++) {
			...
			if (num[i]==0) {
				...
				col = jxl.CellReferenceHelper.getColumnReference(1+i);
				formula = new Formula(1+i, row-7, "MAX("+col+(row+1)+":"+col+(row+365)+")",formatFloat(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));
				sheet.addCell(formula); 
				formula = new Formula(1+i, row-6, "AVERAGE("+col+(row+1)+":"+col+(row+365)+")",formatFloat(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));
				sheet.addCell(formula); 
				formula = new Formula(1+i, row-5, "STDEVP("+col+(row+1)+":"+col+(row+365)+")",formatFloat(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));
				sheet.addCell(formula); 
				formula = new Formula(1+i, row-4, "1282/1000*"+col+(row-5+1)+"+"+col+(row-6+1)+")",formatFloat(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));
				sheet.addCell(formula); 
				...
				for (j=364; j>=0; j--)
					if ( msCoreConfig[i].getDayAvg(j)>0) break;
				addNumber(sheet,1+i,row-3, msCoreConfig[i].getDayAvg(j), formatInt(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));
			}
		...
		for (i=0; procPoolName!=null && i<procPoolName.length; i++) {
			...
			if (num[managedSystem.length+i]==0) {
				...
				col = jxl.CellReferenceHelper.getColumnReference(managedSystem.length+2+i);
				formula = new Formula(managedSystem.length+2+i, row-7, "MAX("+col+(row+1)+":"+col+(row+365)+")",formatFloat(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));
				sheet.addCell(formula); 
				formula = new Formula(managedSystem.length+2+i, row-6, "AVERAGE("+col+(row+1)+":"+col+(row+365)+")",formatFloat(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));
				sheet.addCell(formula); 
				formula = new Formula(managedSystem.length+2+i, row-5, "STDEVP("+col+(row+1)+":"+col+(row+365)+")",formatFloat(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));
				sheet.addCell(formula); 
				formula = new Formula(managedSystem.length+2+i, row-4, "1282/1000*"+col+(row-5+1)+"+"+col+(row-6+1)+")",formatFloat(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));
				sheet.addCell(formula);
				...
				for (j=364; j>=0; j--)
					if ( procPoolConfig[i].getDayAvg(j)>0) break;
				addNumber(sheet,managedSystem.length+2+i,row-3, procPoolConfig[i].getDayAvg(j), formatInt(B_RIGHT_LOW|B_BOTTOM_LOW|B_LEFT_LOW));
			}
		}
	...
	private DataSheet createSysPoolUsageSheet() {
		...
		for (i=0; i<managedSystem.length; i++)
			if ( (gcNames=msCoreUsed[i].getMonthNames()) !=null)
				break;
		...
		for (i=0; i<managedSystem.length; i++) {
			...
			for (j=0; j<gcNames.length; j++) {
				f = msCoreUsed[i].getMonthAvg(j);
				f2 = msCoreConfig[i].getMonthAvg(j);
		...
		for (i=0; i<gcNames.length; i++) {
			s = gcNames[i].get(Calendar.YEAR)+"-"+(gcNames[i].get(Calendar.MONTH)+1)+"-"+gcNames[i].get(Calendar.DAY_OF_MONTH);
			ds.addLabel(1+i,row,s,BOLD|B_ALL_MED);
			ds.addLabel(16+i,row,s,BOLD|VCENTRE|CENTRE|B_ALL_MED);
		}
		row++;
		...
		for (i=0; i<managedSystem.length; i++) {
			...
			for (j=0; j<gcNames.length; j++) {
				f = msCoreUsed[i].getMonthMax(j);
				f2 = msCoreConfig[i].getMonthMax(j);
		...
		for (i=0; i<managedSystem.length; i++) {
			...
			for (j=0; j<gcNames.length; j++) {
				f = msCoreConfig[i].getMonthAvg(j);
				if (f>=0)
					ds.addFloat(1+j,row+i, f, B_RIGHT_LOW|B_LEFT_LOW|B_BOTTOM_LOW);			
			}
		}
		...
		for (i=0; procPoolName!=null && i<procPoolName.length; i++) {
			...
			for (j=0; j<gcNames.length; j++) {
				f = procPoolUsed[i].getMonthAvg(j);
				f2 = procPoolConfig[i].getMonthAvg(j);
		...
		for (i=0; procPoolName!=null && i<procPoolName.length; i++) {
			...
			for (j=0; j<gcNames.length; j++) {
				f = procPoolUsed[i].getMonthMax(j);
				f2 = procPoolConfig[i].getMonthMax(j);
		...
		for (i=0; procPoolName!=null && i<procPoolName.length; i++) {
			...
			for (j=0; j<gcNames.length; j++) {
				f = procPoolConfig[i].getMonthMax(j);
				if (f>=0)
					ds.addFloat(1+j,row+i, f, B_RIGHT_LOW|B_LEFT_LOW|B_BOTTOM_LOW);
			}
		}
	...
	private void createSysPoolUsageSheet(WritableSheet sheet) throws RowsExceededException, WriteException {
		int i,j;
		String s;
		float f,f2;
		int		size=0;
		...
		addLabel(sheet,0,row,"Average Pool Usage",formatLabel(BOLD|CENTRE|VCENTRE|GREEN));
		sheet.mergeCells(15, row, 28, row);
		addLabel(sheet,15,row,"Average Percentage Pool Usage",formatLabel(BOLD|CENTRE|VCENTRE|GREEN));
		row++;
		...
		for (i=0; i<managedSystem.length; i++) {
			...
			for (j=0; j<gcNames.length; j++) {
				...
				wcf = formatFloat(map);	
				addNumber(sheet,1+j,row+i, f, wcf);
				addNumber(sheet,16+j,row+i, f2, wcf);
			...
			wcf = formatLabel(map);	
			addLabel(sheet,0,row+i,s,wcf);
			addLabel(sheet,15,row+i,s,wcf);
			//addLabel(sheet,0,row+i,s,formatLabel(BOLD|VCENTRE|CENTRE|B_ALL_MED));
			//addLabel(sheet,15,row+i,s,formatLabel(BOLD|VCENTRE|CENTRE|B_ALL_MED));
		}
		row+=i;
		...
		sheet.mergeCells(0, row, 13, row);
		addLabel(sheet,0,row,"Peak Pool Usage",formatLabel(BOLD|CENTRE|VCENTRE|GREEN));
		sheet.mergeCells(15, row, 28, row);
		addLabel(sheet,15,row,"Peak Percentage Pool Usage",formatLabel(BOLD|CENTRE|VCENTRE|GREEN));
		row++;
		
		for (i=0; i<gcNames.length; i++) {
			s = gcNames[i].get(Calendar.YEAR)+"-"+(gcNames[i].get(Calendar.MONTH)+1)+"-"+gcNames[i].get(Calendar.DAY_OF_MONTH);
			addLabel(sheet,1+i,row,s,formatLabel(BOLD|B_ALL_MED));
			addLabel(sheet,16+i,row,s,formatLabel(BOLD|VCENTRE|CENTRE|B_ALL_MED));
		}
		row++;
		...
		sheet.mergeCells(0, row, 13, row);
		addLabel(sheet,0,row,"Average Pool Size",formatLabel(BOLD|CENTRE|VCENTRE|GREEN));
		row++;
		...
		sheet.mergeCells(0, row, 13, row);
		addLabel(sheet,0,row,"Average Pool Unassigned",formatLabel(BOLD|CENTRE|VCENTRE|GREEN));
		row++;
		...
		sheet.mergeCells(0, row, 13, row);
		addLabel(sheet,0,row,"Average Virtual Pool Usage",formatLabel(BOLD|CENTRE|VCENTRE|GREEN));
		sheet.mergeCells(15, row, 28, row);
		addLabel(sheet,15,row,"Average Percentage Virtual Pool Usage",formatLabel(BOLD|CENTRE|VCENTRE|GREEN));
		row++;
		...
		for (i=0; procPoolName!=null && i<procPoolName.length; i++) {
			...
			addLabel(sheet,0,row+i,s,wcf);
			addLabel(sheet,15,row+i,s,wcf);			
			//addLabel(sheet,0,row+i,s,formatLabel(BOLD|VCENTRE|CENTRE|B_ALL_MED|WRAP));
			//addLabel(sheet,15,row+i,s,formatLabel(BOLD|VCENTRE|CENTRE|B_ALL_MED|WRAP));
		...
		sheet.mergeCells(0, row, 13, row);
		addLabel(sheet,0,row,"Peak Virtual Pool Usage",formatLabel(BOLD|CENTRE|VCENTRE|GREEN));
		sheet.mergeCells(15, row, 28, row);
		addLabel(sheet,15,row,"Peak Percentage Virtual Pool Usage",formatLabel(BOLD|CENTRE|VCENTRE|GREEN));
		row++;
		...
		sheet.mergeCells(0, row, 13, row);
		addLabel(sheet,0,row,"Average Virtual Pool Size",formatLabel(BOLD|CENTRE|VCENTRE|GREEN));
		row++;
	...
	private DataSheet createLparCoreUsageSheet() {
		...
		for (i=0; i<lparNames.length; i++)
			if ( (gcNames=lparPC[i].getMonthNames()) !=null)
				break;
		...
		for (i=0; i<lparNames.length; i++) {
			...
			for (j=0; j<gcNames.length; j++) {
				f = lparPC[i].getMonthAvg(j);
				f2 = lparEnt[i].getMonthAvg(j);
				cap = lparStatus[i].getMonthCap(j);
				ms = lparStatus[i].getMonthMS(j);
				pool = lparStatus[i].getMonthPool(j);
				vp = lparVP[i].getMonthAvg(j);
				
				if (f<0 || f2<=0)
					continue;
				
				if (pool.equals("DefaultPool")) {
					free = msCoreConfig[getMSid(ms)].getMonthAvg(j) - msCoreUsed[getMSid(ms)].getMonthAvg(j);
				} else {
					poolID = getProcPoolId(ms,pool);
					free = procPoolConfig[poolID].getMonthAvg(j)-procPoolUsed[poolID].getMonthAvg(j);
				}
		...
		for (i=0; i<lparNames.length; i++) {
			...
			for (j=0; j<gcNames.length; j++) {
				f = lparPC[i].getMonthMax(j);
				f2 = lparEnt[i].getMonthMax(j);
		...
		for (i=0; i<lparNames.length; i++) {
			...
			for (j=0; j<gcNames.length; j++) {
				f = lparEnt[i].getMonthAvg(j);
				if (f>=0)
					ds.addFloat(1+j,row+i, f, B_RIGHT_LOW|B_LEFT_LOW|B_BOTTOM_LOW);
				f = lparVP[i].getMonthAvg(j);
				if (f>=0)
	...
	private void createLparCoreUsageSheet(WritableSheet sheet) throws RowsExceededException, WriteException {
		int i,j;
		String s;
		float f,f2;
		int		size=0;
		...
		sheet.mergeCells(0, row, 13, row);
		addLabel(sheet,0,row,"Average Processor Usage",formatLabel(BOLD|CENTRE|VCENTRE|GREEN));
		sheet.mergeCells(15, row, 28, row);
		addLabel(sheet,15,row,"Average Entitlement Percentage Usage",formatLabel(BOLD|CENTRE|VCENTRE|GREEN));
		row++;
		...
		for (i=0; i<lparNames.length; i++) {
			...
			for (j=0; j<gcNames.length; j++) {
				...
				wcf = formatFloat(map);	
				addNumber(sheet,1+j,row+i, f, wcf);
				addNumber(sheet,16+j,row+i, f/f2*100, wcf);
			...
			wcf = formatLabel(map);	
			addLabel(sheet,0,row+i,lparNames[i],wcf);
			addLabel(sheet,15,row+i,lparNames[i],wcf);
		}
		row+=i;
		row++;
		...
		sheet.mergeCells(0, row, 13, row);
		addLabel(sheet,0,row,"Peak Processor Usage",formatLabel(BOLD|CENTRE|VCENTRE|GREEN));
		sheet.mergeCells(15, row, 28, row);
		addLabel(sheet,15,row,"Peak Entitlement Percentage Usage",formatLabel(BOLD|CENTRE|VCENTRE|GREEN));
		row++;
		...
		for (i=0; i<lparNames.length; i++) {
			...
			addLabel(sheet,0,row+i,lparNames[i],formatLabel(BOLD|VCENTRE|CENTRE|B_ALL_MED));
			addLabel(sheet,15,row+i,lparNames[i],formatLabel(BOLD|VCENTRE|CENTRE|B_ALL_MED));
		...
		sheet.mergeCells(0, row, 13, row);
		addLabel(sheet,0,row,"Average Entitlement",formatLabel(BOLD|CENTRE|VCENTRE|GREEN));
		sheet.mergeCells(15, row, 28, row);
		addLabel(sheet,15,row,"Average VPs",formatLabel(BOLD|CENTRE|VCENTRE|GREEN));
		row++;
		...
		for (i=0; i<lparNames.length; i++) {
			...
			for (j=0; j<gcNames.length; j++) {
				...
				f = lparVP[i].getMonthAvg(j);
				if (f>=0)
					addNumber(sheet,16+j,row+i, f, formatFloat(B_RIGHT_LOW|B_LEFT_LOW|B_BOTTOM_LOW));
			}
		}
		row+=i;
		row++;
	...
	private void swapDataManagerObject(Old_DataManager[] array, int a, int b) {
		if (array==null || a<0 || a>array.length || b<0 || b>array.length)
			return;
		
		Old_DataManager dm = array[a];
		array[a]=array[b];
		array[b] = dm;
	}
	
	
	private void swapLparStatusObject(LparStatus[] array, int a, int b) {
		if (array==null || a<0 || a>array.length || b<0 || b>array.length)
			return;
		
		LparStatus dm = array[a];
		array[a]=array[b];
		array[b] = dm;
	}
	...
	private void createLparHourlyImage(int id, File file) {
		...
		for (i=0; i<pc.length; i++) {
			pc[i] = lparPC[id].getHourAvg(i);
			ent[i] = lparEnt[id].getHourAvg(i);
			vp[i] = lparVP[id].getHourAvg(i);
			...
			if (pool.equals("DefaultPool")) {
				free = msCoreConfig[getMSid(ms)].getHourAvg(i) - msCoreUsed[getMSid(ms)].getHourAvg(i);
			} else {
				poolID = getProcPoolId(ms,pool);
				free = procPoolConfig[poolID].getHourAvg(i)-procPoolUsed[poolID].getHourAvg(i);
			}
		...
		for (i=STEP, j=0; i<24*60; i+=STEP) {
			gc = lparPC[id].getHourDate(i);
			if (gc==null)
				label[j++]="";
			else
				label[j++] = gc.get(Calendar.YEAR)+"-"+(gc.get(Calendar.MONTH)+1)+"-"+gc.get(Calendar.DAY_OF_MONTH);
		}
	...
	private void createVirtPoolDailyImage(int id, File file) {
		...
		for (i=0; i<pc.length; i++) {
			pc[i] = procPoolUsed[id].getDayAvg(i);
			size[i] = procPoolConfig[id].getDayAvg(i);
			
			if (size[i]<=msCoreConfig[msID].getDayAvg(i)-msCoreUsed[msID].getDayAvg(i))
				max[i]=size[i];
			else
				max[i] = msCoreConfig[msID].getDayAvg(i)-msCoreUsed[msID].getDayAvg(i);
		}
		
		
		
		for (i=STEP, j=0; i<365; i+=STEP) {
			gc = procPoolUsed[id].getDayDate(i);
			if (gc==null)
				label[j++]="";
			else
				label[j++] = gc.get(Calendar.YEAR)+"-"+(gc.get(Calendar.MONTH)+1)+"-"+gc.get(Calendar.DAY_OF_MONTH);
	...
	private void createVirtPoolHourlyImage(int id, File file) {
		...
		for (i=0; i<pc.length; i++) {
			pc[i] = procPoolUsed[id].getHourAvg(i);
			size[i] = procPoolConfig[id].getHourAvg(i);
			
			if (size[i]<=msCoreConfig[msID].getHourAvg(i)-msCoreUsed[msID].getHourAvg(i))
				max[i]=size[i];
			else
				max[i] = msCoreConfig[msID].getHourAvg(i)-msCoreUsed[msID].getHourAvg(i);
		}
		
		
		
		for (i=STEP, j=0; i<24*60; i+=STEP) {
			gc = procPoolUsed[id].getHourDate(i);
			if (gc==null)
				label[j++]="";
			else
				label[j++] = gc.get(Calendar.YEAR)+"-"+(gc.get(Calendar.MONTH)+1)+"-"+gc.get(Calendar.DAY_OF_MONTH);
	...
	private void createSysPoolHourlyImage(int id, File file) {
		...
		for (i=0; i<pc.length; i++) {
			pc[i] = msCoreUsed[id].getHourAvg(i);
			size[i] = msCoreConfig[id].getHourAvg(i);
		}
		
		
		
		for (i=STEP, j=0; i<24*60; i+=STEP) {
			gc = msCoreUsed[id].getHourDate(i);
			if (gc==null)
				label[j++]="";
			else
				label[j++] = gc.get(Calendar.YEAR)+"-"+(gc.get(Calendar.MONTH)+1)+"-"+gc.get(Calendar.DAY_OF_MONTH);
	...
	private void createSysPoolDailyImage(int id, File file) {
		...
		for (i=0; i<pc.length; i++) {
			pc[i] = msCoreUsed[id].getDayAvg(i);
			size[i] = msCoreConfig[id].getDayAvg(i);					
		}
		
		
		
		for (i=STEP, j=0; i<365; i+=STEP) {
			gc = msCoreUsed[id].getDayDate(i);
			if (gc==null)
				label[j++]="";
			else
				label[j++] = gc.get(Calendar.YEAR)+"-"+(gc.get(Calendar.MONTH)+1)+"-"+gc.get(Calendar.DAY_OF_MONTH);
	...
	private void createLparDailyImage(int id, File file) {
		...
		for (i=0; i<pc.length; i++) {
			pc[i] = lparPC[id].getDayAvg(i);
			ent[i] = lparEnt[id].getDayAvg(i);
			vp[i] = lparVP[id].getDayAvg(i);
			...
			if (pool.equals("DefaultPool")) {
				free = msCoreConfig[getMSid(ms)].getDayAvg(i) - msCoreUsed[getMSid(ms)].getDayAvg(i);
			} else {
				poolID = getProcPoolId(ms,pool);
				free = procPoolConfig[poolID].getDayAvg(i)-procPoolUsed[poolID].getDayAvg(i);
			}
